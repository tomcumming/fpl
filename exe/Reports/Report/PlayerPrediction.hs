...
import FPL.LoadData.Players (PlayerId, loadPlayerData, pdPlayers, psName, psTeam)
...
playerPrediction :: Sv.Server (Sv.Get '[L.HTML] (L.Html ()))
playerPrediction = do
  ...
  let rows =
        M.assocs predicted
          & List.sortOn (snd >>> fmap totalPredicted >>> sum >>> Down)
  ...
  pure $ baseTemplate $ L.table_ $ do
    L.thead_ $ L.tr_ $ do
      ...
      L.th_ "Team"
      L.th_ ""
      forM_ cols $ \mw -> L.th_ $ L.toHtml $ T.show mw
    L.tbody_ $ forM_ rows $ \(pid, mws) -> L.tr_ $ do
